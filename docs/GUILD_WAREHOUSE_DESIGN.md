...
> 範圍：`/公會 倉庫`、`/公會 存入`、`/公會 領取` 三條新指令，並補上設定、紀錄、UI、反洗錢、平衡性等所有相關面向。
...
| `/公會 存入 [物品] [數量]` | 全會員 | ❌（公開） | 把背包 / 魚袋裡的資源捐入倉庫 |
| `/公會 領取 [物品] [數量]` | 全會員 | ❌（公開） | 申請領取倉庫資源，支付手續費入公會金庫 |
...
| id | 顯示 | 單價（config `price`） | 領取單次上限 | 預設備庫格 |
|---|---|---|---|---|
| `stone` | 石頭 | 8 | 15 | 1000 |
| `coal` | 煤炭 | 20 | 15 | 600 |
| `iron` | 鐵礦 | 60 | 10 | 300 |
...
| id | 顯示 | 單價（payout 中位） | 領取單次上限 | 預設備庫格 |
|---|---|---|---|---|
...
「領取單次上限」對應原提議的數字，但**改為「每物品由 config 決定預設值，會長可在上下限內微調」**——見 §6 設定。
...
### 3.2 不可收回
存入即放棄所有權，不提供撤銷。**理由**：避免「存→等隊友幾秒沒看到→自己領回」打亂節奏；同時讓存入在語意上等同捐贈，與「金庫捐款」一致。

### 3.3 新存物品保護期（防掃倉）
每筆 deposit 寫入 `available_at = now + protectionMs`（預設 1 小時）。**保護期內全員都不可領（包含存入者本人）**。
- 倉庫顯示時會把「保護中」的數量單獨標示：`鐵礦 320（其中 15 保護中，<t:…:R> 解鎖）`。
- 計算「可被領取的數量」時，永遠用 `qty - sum(protectedQty)`。

實作上不需逐筆追蹤每顆鐵礦，而是維護 per-item 的「未來解鎖批次表」`pending[]: [{qty, available_at, depositor_id}]`，每次領取前先 `sweep()` 把已到期批次合併進 `available_qty`。

### 3.4 同存入者 24h 自存自領鎖
即使保護期已過，**領取者在過去 24 小時內存過該 item，就不能領這個 item**（純查 `guild_club_warehouse_log` 即可，不需額外欄位）。
- 目的：擋掉「存進去刷一筆貢獻 → 自己領回」的零成本攻擊。
- 不擋「我存了鐵、明天領別人存的鐵」的正常用法，因為次日 day_key 切換、自身 deposit 也滾出 24h 視窗。

### 3.5 不計入任務進度
存入**不會**累加任何個人或公會週任務的計數。理由：避免 A 存 → B 領 → B 存 → A 領的循環刷。但會在「個人公會貢獻值」（顯示在 `/公會 資訊`）加上市價×係數（預設 0.1，可調），讓存的人有面子。
...
「累積貢獻值」= 金庫捐款 + 倉庫存入市價×係數 + 公會 boss 貢獻 + 任務貢獻（複用既有 `guildClubContribution` 欄位即可）。
...
### 4.3 單次數量上限
- 由 config 提供「該物品的上下限」（見 §2 的「領取單次上限」即上限，下限統一為 1）。
...
| `contribution_not_enough` | 🔒 公會貢獻不足 | 目前 X，需 Y | 提示 /公會 捐款 或存入 |
...
8. **底部 ActionRow**：`[存入…] [捐款] [我的貢獻] [紀錄（會長/副會長）]`
   - 「存入…」開 modal 讓玩家填 itemId + qty（autocomplete 在 slash command 較順，此按鈕為 fallback）。
...
| 領取次數 / 上限可被會長亂改破壞平衡 | §6 全部有 `_Range`，超出範圍直接拒 |
...
| `guildClubAnnouncer` | 大額存入（單次市價 ≥ 10,000）廣播到公會頻道，與「升級」廣播風格一致 |
...
### M1 — MVP（建議 2 週內完成）
- §3 存入、§4 領取、§5.1~5.3 三個 UI、§7 三張 collection、§9 全部反洗錢檢查。
- `/公會 倉庫設定` 只暴露 `dailyMaxTimes`、`feeRate`、`tenureHours`、`minContribution` 四項（最常被微調的）。

### M2 — 監督與微調（✅ 已實作）
- `/公會 倉庫紀錄`、§9 連續外流警示、§10 大額廣播。
- `/公會 倉庫設定` modal（4 項：每日次數、手續費率、入會時間、最低貢獻），上下限超出直接拒。
- `/公會 資訊` 新增倉庫摘要區塊（總價值 / 最近活動 / 近 7 天淨流）+ 待審申請按鈕。
- §12 三條指令收斂：編輯簡介、申請列表、領獎已全部改為按鈕。Slash command 由 19 → 16 條。
- 每物品 perTakeMax 微調：留 M3（M2 觀察期內先不開放，避免會長亂調破壞平衡）。
...
**收斂後 slash command 從 21 → 12 條**：建立、資訊、解散、邀請、申請、退會、捐款、任務、排行、倉庫、存入、領取。
...
- **任務 / 排行 / 倉庫 / 存入 / 領取**：高頻查詢與行動入口，留 slash 便於肌肉記憶。
