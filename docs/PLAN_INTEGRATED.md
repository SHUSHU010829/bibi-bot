...
>
> 最後更新：2026-06-03（Phase A 公會系統完成）
...
| Phase A | 公會系統 | ✅ 已實作（2026-06-03） | 本文件 |
...
| **新生產 → 必配對應 sink** | 每個新增的生產類玩法（產幣 / 產物），上線時必須同時設計對應的消耗出口；每個新 Phase 章節必須有「消耗出口」獨立段落 |
| **buff 統一彙整** | 所有屬性加成透過 `buffResolver`（見 `PLAN_OPTIMIZATION.md` Opt-5） |
| **跨系統走 eventBus** | 新玩法之間的觸發以 `src/features/eventBus` 訂閱事件為主，不在既有 service 裡寫死 hook |
| **設定檔驅動** | 平衡參數放 JSON，不寫死在程式 |
| **命名風格一致** | 頻道名稱維持「地名 / 場所感、五個字」風格 |

### 2.1 銷毀槽（sink）設計原則

> 鐵律：**新增生產來源 → 同 Phase 內必須交付對應的消耗出口**。
> 否則新產出會貶值、或讓既有幣 / 物資進一步通膨。

| 生產來源（新水龍頭） | 必配的消耗槽 |
|---|---|
| 動物畜牧（產蛋 / 奶 / 毛） | 料理 / 賣店 / 寵物餵食 |
| 遺跡考古（產文物） | 小屋展示 / 收藏家 NPC 收購 |
| 市集擺攤（產幣） | 擺攤費 / 攤位裝飾消費 |
| 寵物餵食 | 已是 fishing / farm 的天然 sink ✅ |
| 領地建造 / 鍊金術 | 本身就是礦石銷毀槽 ✅ |

**排程含義**：
- 「sink 型 Phase」（寵物、鍊金術、領地建造）應**優先於**「水龍頭型 Phase」（畜牧、考古）
- 既有 ⭐⭐⭐ 玩法的排程順序也受此原則約束：A1 寵物 早於 F3 畜牧 / C2 考古

每個新 Phase 撰寫時，**「消耗出口」段落為必填**；缺少這段視同企劃未完成、不進入排程。
...
**MVP 階段：每週六 1 場固定刷新**，先跑 2–4 週觀察數據再決定是否加場。

| 線上玩家參與率 | 後續調整 |
|---|---|
| > 60% | 加開「驚喜場」（每週隨機 1–2 次、無預告、血量 ×200、僅給幣不給稀有素材） |
| 30%–60% | 維持每週 1 場、優化獎勵分配 |
| < 30% | 改成每兩週 1 場、提高血量與 Top 3 獎勵 |

> 為什麼不一開始就排密集場次：稀有素材獎勵會貶值、社群儀式感會稀釋。先建立「週六 21:00 = 逼逼來了」的習慣，再依數據加密。
...
// 額外：最後一擊（屠龍者尾刀獎勵）
//   - 動態幣：floor(BOSS_HP × 0.05)
//   - 1 個稀有素材（從 Top 3 池抽）
//   - 屠龍者擊殺數 +1（累積制，未來可解鎖進階稱號 e.g. 屠龍 ×10 → 龍裔）
```
...
| 場次 | BOSS_HP | 尾刀幣 | 稀有素材 |
|---|---|---|---|
| 週六大場（30 人線上） | 15,000 | 750 | ✅ |
| 平日驚喜場（線上 ×200） | 6,000 | 300 | ✅ |
| 節日特殊場 | 50,000 | 2,500 | ✅ |
...
| 稱號 | 條件 | 性質 |
|---|---|---|
| **屠龍者** | 對 BOSS 造成最後一擊 | 永久 + 累積擊殺數（×10 解鎖進階稱號「龍裔」） |
| **萬人敵** | 單場傷害 Top 1 | 月榜輪替 |
...
```json
{
  ...
  "rewards": {
    "poolRatio": 0.5,
    "killBonus": 100,
    "killerBonusRatio": 0.05,
    "killerRareReward": 1,
    "topRareRewards": 3,
    "dragonSlayerTitleThreshold": 10
  },
  "counterMessages": [
    "BOSS 一巴掌把你打飛！",
    "你揮空了，BOSS 還順便嗆了你一句。"
...
| Phase A | 公會系統 | 5–7 天 | Phase 5、Opt-5 | ✅ 已完成（2026-06-03） |
| **合計** | | **27–38 天** | | |
