...
指令：`/股市`（`買` / `賣` / `走勢` / `配息` / `報價` / `持股` / `紀錄` 子指令）、`/stock-event`🔒。
...
| --- | --- |
| `/股市 買 股票代號 數量` | 市價買入 |
| `/股市 賣 股票代號 數量` | 市價賣出（數量可 `all`） |
| `/股市 走勢 股票代號 [期間]` | 單股歷史走勢圖（`1d` / `1w` / `1m`，預設 `1w`） |
| `/股市 配息 [期間]` | 個人配息明細（`1m` / `3m`，預設 `1m`） |
| `/股市 報價` | 互動報價面板，下拉選股 + 按鈕直接買 / 賣 / 看走勢 |
| `/股市 持股` | 個人持倉、損益與一鍵賣出 |
| `/股市 紀錄 [期間] [股票代號]` | 個人買賣成交歷史與已實現損益（`1w` / `1m` / `3m`，預設 `1m`） |
| `/stock-event ...` 🔒 | 開發者：觸發 / 新增 / 移除突發事件 |

> `/股市 買`、`/股市 賣`（含 `報價` 面板的買賣按鈕）只在開盤時間（09:00–21:00）可用，且完成任一筆即標記 `daily_stock` 任務。
> `配息` / `紀錄` 紀錄皆保留 90 天（`StockTransactions` 90 天 TTL）。
...
- `/稱號 設定`：可選任一已解鎖徽章、**已解鎖的遊戲稱號**（挖礦 / 賭場 / 股市 / 樂透 / 拍賣），或目前等級 tier（還原預設）
- `/檔案`（成就分頁）：查看等級徽章 + 遊戲稱號的完整圖鑑與解鎖進度
- `/稱號 展示徽章 / 重置展示`：自選等級卡下方顯示 5 枚徽章與順序
...
- 「賭桌新手」每日任務以 `bet` 為觸發
- `/排行榜 類別:賭場贏家 / 賭場輸家`：可切今天 / 本週 / 本月
- `/檔案`（賭場紀錄分頁）：個人下注、派彩、RTP、各遊戲分項；`/casino-stats`🔒 為全域統計
...
指令 `/拉霸 [下注] [梭哈]`，設定 `casino.slot`

### 18.0 機制概要

3 欄 × 3 列共 9 格、**5 條 payline**：3 條橫排 + 2 條對角線。

- 下注金額 = 全部 5 條線的總注，每條線 bet = `floor(bet / 5)`（至少 1）。
- 每條 payline 獨立判定（三連 / 兩連 / 兩櫻桃），多條中獎賠率相加。
- **任一條 payline 連到 7️⃣7️⃣7️⃣** 就觸發 JACKPOT，整池額外加碼。
...
總權重 = 100；單格機率 = weight / 100。3×3 全部 9 格各自獨立抽取。

### 18.2 Payline 定義（PAYLINES）

| id | 名稱 | cells `[row,col]` |
| --- | --- | --- |
| `row_top` | 上排 | `[0,0] [0,1] [0,2]` |
| `row_mid` | 中排 | `[1,0] [1,1] [1,2]` |
| `row_bot` | 下排 | `[2,0] [2,1] [2,2]` |
| `diag_dn` | 左上對角 | `[0,0] [1,1] [2,2]` |
| `diag_up` | 左下對角 | `[2,0] [1,1] [0,2]` |

### 18.3 單條 payline 倍率（套用 per-line bet）
...
每條 payline 的賠付 = `floor(perLineBet × multiplier)`，總賠付 = 各線加總。

### 18.4 Jackpot Pool 邏輯（`features/casino/slot/jackpotPool.js`）

```
每筆下注：pool += floor(bet × 0.03)
任一 payline 命中 7️⃣7️⃣7️⃣：
  base payout = 各線 payout 加總（含該條 ×450）
  jackpot 加碼 = max(0, pool − seedAmount)
  pool 重置為 5,000
  在 announceChannelId 推播
```

> 此次改成 3×3 時 **保留現有 `JackpotPool` collection**：因為 collection 以 `(guildId, game="slot")` 為唯一鍵，累積中的 pool 不會被清空。

**目標 RTP**：≈ 82–86%（含通膨控制；多條 payline 的命中率提升以較小的 per-line bet 平衡）。
