...
- **早安卡**：`events/ready/sendMorningMessage.js` 依 `morningMessage.cronSchedule`（預設每天早上 8 點）發送，使用 `satori` 動態繪製含日期、星期、節氣、農民曆、詩詞、運勢的 PNG 卡片。倒數區塊（`utils/findNextSpecialDay.js`）平時顯示「距離下一個節日還有 N 天」；當天本身就是節日時（`daysUntilSpecialDay === 0`）改為慶祝排版直接呈現當天節日名稱。
...
> 📍 **可用頻道**：市民服務處 │ 🏢（`citizen` 桶）。標 🔒 的 `food-admin` 指令不受頻道限制。
...
> 📍 **可用頻道**：依資料夾分流——`draw` / `ask` / `recommendation` / `post` 屬 `citizen` 桶（市民服務處 │ 🏢）；`stats` / `leaderboard` / `event` / `quiz` / `general`（`/help`）屬 `general` 桶（`1511655301953228800`）。標 🔒 的管理員指令不受頻道限制。
...
> 📍 **可用頻道**：`general` 桶（`1511655301953228800`）。標 🔒 / 🔧 的管理員 / 開發者指令不受頻道限制。
...
| `/dev level …` / `/dev daily …` 🔧 | 開發者測試工具（XP 給予、升等卡預覽、簽到卡預覽 / 重置） |
...
> 📍 **可用頻道**：`general` 桶（`1511655301953228800`）。標 🔒 的管理員指令不受頻道限制。
...
| `/賣出 [物品] [數量]` | 把礦石或魚賣給系統換金幣，依當日行情計價；不指定數量則賣出全部 🪙 |
...
- **背包**：基礎 100 格（`backpackBaseSlots`），裝滿就不能再挖，要先 `/賣出`。
...
- **金流串接**：`/賣出` 走 `grantCoins` 的 `source: "mining_sell"`、`/打工` 走 `source: "work"`，皆納入經濟日報的 inflow 統計。
...
| `/市集 競標` | 拍賣行：掛牌 24h、成交抽 5% 手續費、最低起標 = 收購價 80%、每人最多掛 5 件；被超越自動退款，到期無人出價退回礦石；掛牌可選填一口價（≥ 起標價），出價達標立即成交（封頂為一口價）🏷️ |
...
| `citizen` | `ask` / `draw` / `food` / `post` / `recommendation` / `roles` / `ticket` / `weather` | 市民服務處 │ 🏢（`1192888968748994700`） |
| `general` | 其餘所有資料夾 | （`1511655301953228800`） |
