...
│   ├── shop/               # /商店、/背包
│   ├── stats/              # /stats
│   ├── stock/              # /買股、/賣股、/股歷、/配息紀錄、/stock-event
...
| `/補簽卡` | 查補簽卡庫存與規則 |
| `/檔案 [分頁] [用戶] [私密]` | 個人檔案聚合：等級卡 / 礦工 / 錢包 / 背包 / 賭場紀錄 / 持股 / 成就（按鈕切分頁，敏感分頁自動 ephemeral） |
...
| 指令 | 用途 |
| --- | --- |
| `/檔案 分頁:錢包` | 查當前 credits、生命總值、來源分布、生效中 buff（從個人檔案的錢包分頁進入） |
...
賭場類遊戲共用同一套節流 / 對帳機制：每款遊戲在 `casino/<game>` 區塊獨立設 `minBet`、`maxBet`，下注走 `source: "bet"`、派彩走 `source: "payout"`，所以 `/檔案 分頁:賭場紀錄` 與 `/排行榜 類別:賭場贏家` 才能算 RTP。
...
| `/檔案 分頁:賭場紀錄` | 自己的下注、派彩、RTP、各遊戲分項統計 | — |
...
| `/檔案 分頁:持股` | 查看當前持股、損益、總市值（從個人檔案的持股分頁進入） |
...
| 指令 | 用途 |
| --- | --- |
| `/挖礦` | 挖一次礦（冷卻 2 小時）。挖到鑽石會全服公告 ✨ |
| `/背包` | 查看礦石、容量、目前鎬子耐久、道具（幸運藥水 / CD 縮短券 / 傳說素材碎片）與下次可挖時間；冷卻中可在此使用 CD 縮短券 🎒 |
| `/賣礦 [礦石] [數量]` | 把礦石賣給系統換金幣；不指定礦石則一次賣出背包全部 🪙 |
| `/打工` | 打工賺穩定金幣（冷卻 4 小時，每日最多 6 次）💼 |
| `/合成 裝備 [確認]` | 用礦石合成更好的鎬子；要替換還有耐久的鎬子需把 `確認` 設成 `true` 🔨 |
| `/裝備` | 查看目前鎬子屬性，以及各配方的材料進度（✅ / ❌）🔧 |
...
| 指令 | 用途 |
| --- | --- |
| `/檔案 分頁:成就` | 查看等級徽章 + 遊戲稱號的完整圖鑑與進度（⭐ 展示中 / ✅ 已解鎖 / 🔒 未解鎖）🏆 |
| `/稱號 設定` | 切換等級卡稱號（徽章 / 等級 tier / 遊戲稱號）|
| `/檔案 分頁:礦工檔案 [用戶]` | 礦工生涯統計、歷史採集量、週冠次數、目前展示稱號 📜 |
