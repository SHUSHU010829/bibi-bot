...
**武器與戰鬥**（`dungeon.weapons` / `craft.recipes` 中 `result.type = "weapon"`，戰鬥力與打怪硬綁定，與鎬子分離；非赤手有耐久，打怪時消耗，耗盡退回赤手）：
...
| 指令 | 用途 |
| --- | --- |
| `/地下城` | 消耗 1 點體力進地下城戰鬥（體力上限 10、每小時回 1）；**需先 `/合成` 打造武器（赤手無法打怪）**，勝率由武器戰鬥力決定、高階武器有暴擊（保證獲勝＋金幣 ×1.5），勝利掉落礦石 / 金幣 / 幸運藥水 / CD 縮短券 / 傳說素材碎片，背包滿時礦石折金幣 ⚔️ |
...
- **體力**：`/地下城` 每場耗 1 點、0 點不能進；惰性回復（每小時 +1，上限 10）。**戰鬥力 = 基礎 20 + 武器加成**（赤手 0 / 鐵劍 +25 / 鋼劍 +50 / 黃金劍 +80 / 鑽石劍 +120 / 傳說之劍 +180；鎬子不再影響戰鬥），怪物 HP 60–280，勝率 = `clamp(戰鬥力 / 怪物HP, 0.2, 0.9)`，暴擊則直接獲勝。武器每場耗 1 耐久，耗盡退回赤手。
